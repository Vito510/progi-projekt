...
export default class RayCast {
    ...
    static rayCast(heightmap, coordinates, uniforms) {
        ...
        const temp = new Vector3D(centered_coordinates.x * uniforms.fov, 1.0, centered_coordinates.y * uniforms.fov);
        const direction = Matrix.mul(uniforms.camera_rotation, new Vector4D(temp.x, temp.y, temp.z, 1.0)).xyz();
    ...
    static #traverse(heightmap, uniforms, origin, direction) {
        ...
        const bbox_t = RayCast.#intersect(origin, direction, new Vector3D(), Vector3D.mul(uniforms.grid_size, uniforms.grid_scale));
    ...
    static #getHeight(heightmap, uniforms, position) {
        const coordinates = Vector2D.mul(Vector2D.sub(new Vector2D(1.0, 0.0), Vector2D.div(position.xy(), Vector2D.mul(uniforms.grid_size.xy(), uniforms.grid_scale))), new Vector2D(1.0, -1.0));
        const temp = Vector2D.mul(coordinates, new Vector2D(heightmap.width, heightmap.height));
        const data = ImageUtils.get(heightmap, temp.x, temp.y);
        const height = (data.r * 256.0 + data.g) * 256.0;
